...
bool cMB(unsigned int C[], unsigned int Stock[], unsigned int n, unsigned int T, unsigned int usedCoins[]) {
    if(T==0){
        return true;
    }
    for(int i = n-1; i >= 0; --i){
        if(Stock[i] > 0 && C[i] <= T){
            Stock[i]--; usedCoins[i]++;
            if(cMB(C,Stock,n,T-C[i],usedCoins)){
                return true;
            }
            Stock[i]++; usedCoins[i]--;
        }
...
bool changeMakingBacktracking(unsigned int C[], unsigned int Stock[], unsigned int n, unsigned int T, unsigned int usedCoins[]) {
    ...
    return cMB(C,Stock,n,T,usedCoins);
}
