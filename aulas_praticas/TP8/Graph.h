...
void Graph<T>::fordFulkerson(T source, T target) {
    resetFlows();
    Vertex<T> * v_source = findVertex(source);
    Vertex<T> * v_target = findVertex(target);
    double tot = 0;
    while (findAugmentationPath(v_source, v_target)){
        double f = findMinResidualAlongPath(v_source, v_target);
        augmentFlowAlongPath(v_source, v_target, f);
        tot += f;
    }
}
...
bool Graph<T>::findAugmentationPath(Vertex<T> *s, Vertex<T> *t) {
    ...
    while(!queue.empty() && !(t->visited)){
        Vertex<T> *v = queue.front(); queue.pop();
