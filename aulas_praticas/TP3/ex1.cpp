...
Result nearestPoints_BF(std::vector<Point> &vp) {
    Result res;
    double min_dist = (double)1e9; // TODO: This may be stupid
    for(int i = 0; i < vp.size()-1; ++i){
        for(int j = i+1; j < vp.size(); ++j){
            double new_dist = vp[i].distance(vp[j]);
            if(new_dist < min_dist){
                res = Result(new_dist, vp[i], vp[j]);
                min_dist = new_dist;
            }
        }
    }
...
Result _nearestPoints_DC(const std::vector<Point> &P, int a, int b, std::vector<Point> &Q){
    if(b-a == 1){
        return {P[a].distance(P[b]), P[a], P[b]};
    }
    if(a==b){
        return {};
    }
    std::vector<Point> Ql, Qr;

    int midpoint = a + (b-a)/2;
    double middle_x = P[midpoint].x;
    ...
    Result res;
    Result resultLeft = _nearestPoints_DC(P, a, midpoint, Ql);
    Result resultRight = _nearestPoints_DC(P, midpoint, b, Qr);
    res = resultLeft.dmin < resultRight.dmin ? resultLeft : resultRight;

    std::vector<Point> strip;
    for(auto point : Q){
        if(std::abs(point.x-middle_x) < res.dmin){
    ...
    for(int i = 0; i < strip.size(); ++i){
        for(int j = i + 1; j < strip.size(); ++j){
            if(std::abs(strip[i].y - strip[j].y) > res.dmin){
                break;
            }
            double dist = strip[i].distance(strip[j]);
            if(dist < res.dmin){
                res = Result(dist, strip[i], strip[j]);
            }
        }
    }
    return res;
...
Result nearestPoints_DC(std::vector<Point> &vp) {
    std::vector<Point> P = vp;
    sortByX(P, 0, P.size()-1);
    std::vector<Point> Q = P;
    sortByY(Q, 0, Q.size()-1);
    return _nearestPoints_DC(P, 0, P.size(), Q);
}

Result _nearestPoints_DC_MT(const std::vector<Point> &P, int a, int b, std::vector<Point> Q, int nThreads){
    if(b-a == 1){
        return {P[a].distance(P[b]), P[a], P[b]};
    }
    if(a==b){
    ...
    if(nThreads >= 2){
        std::future<Result> futureLeft = std::async(std::launch::async, _nearestPoints_DC_MT, P, a, midpoint, Ql, nThreads/2);
        std::future<Result> futureRight = std::async(std::launch::async, _nearestPoints_DC_MT, P, midpoint, b, Qr, nThreads/2);
        resultLeft = futureLeft.get();
        resultRight = futureRight.get();
    }
    else{
        resultLeft = _nearestPoints_DC(P,a,midpoint,Ql);
        resultRight = _nearestPoints_DC(P,midpoint,b,Qr);
    }

    Result res = resultLeft.dmin < resultRight.dmin ? resultLeft : resultRight;
